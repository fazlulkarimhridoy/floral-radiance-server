...
router.post("/add-order", async (req, res) => {
  try {
    ...
    // items field is stored as JSON string in DB
...
router.patch("/update-order/:id", async (req, res) => {
  try {
    ...
    // items field is stored as JSON string in DB
    if (orderUpdateData.items && typeof orderUpdateData.items !== "string") {
      orderUpdateData.items = JSON.stringify(orderUpdateData.items);
    }
  ...
  try {
    ...
    const stats = {
      ...
      pending: {
        ...
        amount:
          grouped.find((g) => g.orderStatus === "PENDING")?._sum.totalPrice ||
          0,
      },
      shipped: {
        count: grouped.find((g) => g.orderStatus === "SHIPPED")?._count || 0,
        amount:
          grouped.find((g) => g.orderStatus === "SHIPPED")?._sum.totalPrice ||
          0,
      },
      cancelled: {
        count: grouped.find((g) => g.orderStatus === "CANCELLED")?._count || 0,
        amount:
          grouped.find((g) => g.orderStatus === "CANCELLED")?._sum.totalPrice ||
          0,
      },
      delivered: {
        count: grouped.find((g) => g.orderStatus === "DELIVERED")?._count || 0,
        amount:
          grouped.find((g) => g.orderStatus === "DELIVERED")?._sum.totalPrice ||
          0,
      },
    };
