...
function isAllowedImageExtension(ext) {
    ...
    return ["jpg", "png", "webp", "gif", "svg"].includes(String(ext || "").toLowerCase());
}
    ...
    try {
        ...
        try {
            result = await prisma.product.create({
                data: { ...productData, images: uploadedImages },
            });
        } catch (dbError) {
...
router.patch("/update-product/:id", async (req, res) => {
    try {
        ...
        if (isNaN(productId) || productId < 1) {
            return res.status(400).json({ status: "fail", data: "Invalid product ID" });
        }
        ...
        const addImages = Array.isArray(req.body.addImages) ? req.body.addImages : [];
        ...
        const hasImagesProp = Object.prototype.hasOwnProperty.call(req.body, "images");
        if (hasImagesProp) {
        ...
        // Only set images if something changed (reference check is fine here)
